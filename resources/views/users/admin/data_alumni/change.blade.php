  ...
  <div class="card-header">
    ...
    <div class="card-tools">
      <div class="input-group input-group-sm">
        <a href="{{url('/')}}/admin/data_alumni" class="btn btn-warning"><i class="fa fa-chevron-left"></i>&nbsp;Daftar Alumni</a>
      </div>
    </div>
  </div>
  <div class="card-body">
    <form class="striped-rows" enctype="multipart/form-data" action="{{url('/')}}/admin/data_alumni/update" method="post">
      {{csrf_field()}}
      <div class="row">
        <div class="col-md-6">
          <div class="form-group">
            <label>Nama Alumni</label>
            <input type="text" name="id" value="{{$data_alumni->id}}" hidden>
            <input name="nama_alumni" type="text" class="form-control" placeholder="Masukan nama jurusan" value="{{$data_alumni->nama_alumni}}" required>
          </div>
        </div>
        <div class="col-md-6">
          <div class="form-group">
            <label>Sekolah</label>
            <input name="sekolah_id" type="text" class="form-control" placeholder=""  value="{{$data_alumni->sekolah_id}}" required>
          </div>
        </div>
      </div>
      <hr>
